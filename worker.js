...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/gemini/status") return Response.json({ configured: false, model: MODEL });
    if (url.pathname === "/api/gemini/analyze" && request.method === "POST") {
      try { return Response.json(await gemini(await request.json())); }
      catch (error) { return Response.json({ error: error.message || "분석 중 문제가 생겼습니다." }, { status: 400 }); }
    }
